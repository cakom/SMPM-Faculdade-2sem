...
const product = require("../models/Product"); // Importa o modelo User (Mongoose)

//CREATE: cria um novo Produto
router.post("/", async (req, res) => {// POST /api/users
    const product = await Product.create(req.body); // cria documento no Mongolti com os dados de body
    res.json(product);// Retorna o usuario criado em JSON
});// Fim da rota POST

// READ: lista todos os Produtos
router.get("/", async (req, res) => { // GET /apl/products 
    const products = await  Product.find(); // Busca todos os documentos da coleção
    res.json(products);// Retorna o array de Produtos
});// Fim da rota GET (lista)

// UPDATE: atualiza un Produto por ID
router.put ("/:id", async (req, res) => { // PUT/api/products/:id
    const product = await Product.findByIdAndUpdate( // Atualiza o documento com o ID informado
...
router.delete("/:id", async (req, res)=> { // DELETE /api/products/tid
    await Product.findByIdAndDelete(req.params.id); // Remove o documento com o ID informado
